...
const TimeSegmentSelector = ({ navigation, route }) => {
  ...
  const handleTimeConfirm = () => {
    ...
    if (isToday() && isTimeInFuture(tempHour, tempMinute, tempPeriod)) {
      Alert.alert('Invalid Time', 'Please choose a time that has already passed today.', [
        { text: 'OK' },
      ]);
      return;
    }
  ...
  const handleContinue = () => {
    ...
    if (rememberTime === null) {
      Alert.alert(
        'Please choose',
        isToday() ? 'Choose how to set the time.' : 'Do you remember the specific time or not?'
      );
      return;
    }
    ...
    if (isToday() && (timeType === 'specific' || timeType === 'current')) {
      if (isTimeInFuture(customHour, customMinute, customPeriod)) {
        Alert.alert(
          'Invalid Time',
          'You cannot select a time in the future. Please choose a time that has already passed.'
        );
        return;
      }
    }
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }}>
      ...
      >
            ...
            >
              <View className="items-center">
                <Ionicons
                  name="time-outline"
                  size={26}
                  color={rememberTime === true ? '#15803D' : '#4B5563'}
            ...
            >
              <View className="items-center">
                <Ionicons
                  ...
                  size={26}
                  color={rememberTime === false ? '#1D4ED8' : '#4B5563'}
                  style={{ marginBottom: 8 }}
                />
                <Text
                  className={`font-medium ${
        ...
        {rememberTime === false && (
          <View className="mb-8">
            ...
            {isToday() && (
              <View
                className="mb-4 p-3 rounded-xl flex-row items-center justify-center"
                style={{
                  ...
                  >
                    <View className="flex-row items-center">
                      <Ionicons
                        ...
                        size={26}
                        color={isDisabled ? '#9CA3AF' : colors.primary}
                        style={{ marginRight: 12, opacity: isDisabled ? 0.5 : 1 }}
                      />
        ...
        {rememberTime === true && (
          <View className="mb-8">
            ...
            <TouchableOpacity
              ...
              style={{
                backgroundColor:
                  selectedTime === 'specific' ? 'rgba(78, 205, 196, 0.1)' : 'white',
                borderColor: selectedTime === 'specific' ? '#4ECDC4' : '#B0E5E1',
              }}
              activeOpacity={0.8}
            >
              <View className="flex-row items-center justify-center">
        ...
        >
          ...
          >
            {rememberTime === null
              ? 'Choose if you remember the time'
              : rememberTime === true && !selectedTime
              ? 'Set specific time'
              : rememberTime === false && !selectedTime
        ...
        {/* Helper Text */}
        <View className="mt-6 items-center flex-row justify-center px-4">
          <Ionicons
            name="bulb-outline"
            size={18}
            color={colors.text}
          ...
          />
          <Text
            className="text-sm text-center opacity-80 flex-1"
            style={{
              color: colors.text,
              fontFamily: fonts.regular,
            }}
          >
            This helps us accurately track when your mood/activity occurred for better
            insights
          </Text>
        </View>
      </ScrollView>
      ...
      {showTimeModal && (
        <Modal transparent animationType="fade">
          ...
          >
            <View className="rounded-2xl p-6 m-4 w-80" style={{ backgroundColor: colors.background }}>
              <Text
                className="text-xl text-center mb-6"
                style={{
                  color: colors.text,
              ...
              {isValidTempTime() &&
                isToday() &&
                isTimeInFuture(tempHour, tempMinute, tempPeriod) && (
                  <View
                    className="mb-4 p-3 rounded-xl"
                    style={{
                      backgroundColor: 'rgba(239, 68, 68, 0.1)',
              ...
              <View className="flex-row gap-3">
                ...
                <TouchableOpacity
                  ...
                  disabled={
                    !isValidTempTime() ||
                    (isToday() && isTimeInFuture(tempHour, tempMinute, tempPeriod))
                  }
                  className="flex-1 rounded-xl py-3"
                  style={{
                    backgroundColor:
