...
function getMoodMessage(activity, moodScore, idx = 0) {
...
function getSleepMessage(avgHours, moodScore) {
  const absScore = Math.abs(moodScore);
  if (moodScore > 0) {
...
export default function WeeklyAnova() {
  ...
  useEffect(() => {
    const fetchWeeklyAnova = async () => {
      setLoading(true);
      try {
        const data = await moodDataService.getWeeklyAnova(start.toISOString(), end.toISOString());
        setResults(data.results || {});
        setSleepQuality(data.sleepQuality);
        setAvgSleepHours(data.avgSleepHours);
